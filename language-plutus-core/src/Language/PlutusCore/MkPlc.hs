...
mkVar :: a -> VarDecl tyname name a -> Term dyn tyname name a
...
type TermDef dyn tyname name a = Def (VarDecl tyname name a) (Term dyn tyname name a)
...
mkTermLet
    :: a
    -> TermDef dyn tyname name a
    -> Term dyn tyname name a -- ^ The body of the let, possibly referencing the name.
...
mkIterApp
    :: a
    -> Term dyn tyname name a -- ^ @f@
    -> [Term dyn tyname name a] -- ^@[ x0 ... xn ]@
    -> Term dyn tyname name a -- ^ @[f x0 ... xn ]@
mkIterApp x = foldl' (Apply x)

-- | Make an iterated instantiation.
mkIterInst
    :: a
    -> Term dyn tyname name a -- ^ @a@
    -> [Type tyname a] -- ^ @ [ x0 ... xn ] @
    -> Term dyn tyname name a -- ^ @{ a x0 ... xn }@
